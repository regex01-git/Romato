...
import { add,increaseMenu,decreaseMenu,total } from '../features/Cart';
...
function MenuCard({item}) {
  ...
  let index=cart.cartItems.findIndex((single)=>single._id==item._id)
    function incre(item){
      dispatch(increaseMenu(item));
    }
    function decre(item){
      dispatch(decreaseMenu(item));
    }
    function addItem(){
      dispatch(add(item))
    }
  ...
  return (
    ...
    <Card style={{border:"none",boxShadow:"0 4px 8px rgba(0, 0, 0, 0.4)"}}>
      ...
      {/* <Card.Img variant="top" src="holder.js/100px180" /> */}
       <Col xs={12} style={{backgroundImage: `url(${item.image.url})`,objectFit:"cover",width:"auto",height:"300px",backgroundPosition:"center",backgroundSize:"cover"}} className='d-flex align-items-end justify-content-end'>
        {
